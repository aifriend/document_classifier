...
class TextClass:
    ...
    def train_by(self, model, data, nlp, dictionary):
        ...
        if nlp:
            ...
            if model == AggregateFactory.BAGGING:
                self.logger.Information(f'TextClass - training {model}...')
                response = self.aggregate.launch_bagging_classifier(X_train, y_train, X_test, True)
                self.aggregate.show_metrics(y_test, response, stats=stats)

            else:
                response = []
                stats.info = 'Unsuitable training model. ' \
                             'Should be one of: BAGGING'
    ...
    def predict_by(self, model, nlp, data=None, file_data=None, file_url=None):
        ...
        if nlp:
            ...
            if model == AggregateFactory.BAGGING:
                ...
                response = self.aggregate.launch_bagging_classifier(None, None, X_test, False)
                self.aggregate.show_metrics(None, response, stats=stats)
    ...
    def check_model(model=None):
        ...
        if not (model == AggregateFactory.BAGGING):
            return False
