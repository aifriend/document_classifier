...
from classifier.aggregate.Bagging import Bagging
...
class AggregateFactory:
    ...
    def launch_bagging_classifier(self, X_train=None, y_train=None, X_test=None, train=False):
        self.logger.Information('---< Bagging IClassify >---')
        result = ''
        if train and X_train is not None and y_train is not None and X_test is not None:
            result = self._bagging.train(X_train, y_train, X_test)
        elif not train and X_test is not None:
            result = self._bagging.predict(X_test)
