...
from classifier.basic.ExtraTree import ExtraTree
...
class ClassifyFactory:
    ...
    def __init__(self, conf):
        self.conf = conf
        self._extra_tree = ExtraTree(conf)
    ...
    def launch_extra_trees(self, X_train=None, y_train=None, X_test=None, train=False):
        self.logger.Information('---< Extra Trees >---')
        result = ''
        if train and X_train is not None and y_train is not None and X_test is not None:
            result = self._extra_tree.train(X_train, y_train, X_test)
        elif not train and X_test is not None:
            result = self._extra_tree.predict(X_test)
