...
class TextClass:
    ...
    def predict_by(self, model, nlp, data=None, file_data=None, file_url=None):
        ...
        if nlp:
            ...
            # Basic classifiers
            if model == ClassifyFactory.EXTRA_TREES:
                self.logger.Information(f'TextClass - prediction {model}...')
                response = self.classify.launch_extra_trees(None, None, X_test, False)
    ...
    def check_model(model=None):
        ...
        if not (model == ClassifyFactory.EXTRA_TREES):
            return False
