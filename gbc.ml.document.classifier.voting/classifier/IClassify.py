...
class IClassify(object):
    ...
    def get_prediction(self, x):
        ...
        try:
            ...
            self.logger.Error('IClassify::get_prediction::{}'.format(e), sys.exc_info())
            return None
    ...
    def load_model(self, model_name):
        try:
            self.logger.Information(f'IClassify::loading \'{model_name}\' model...')
            path = os.path.join(self.conf.working_path, model_name)
            self.clf = ClassFile.load_model(path)
        except Exception as e:
            self.logger.Error('IClassify::LOAD::{} - model -> {}'.format(e, model_name), sys.exc_info())
            raise FileNotFoundError

    def save_model(self, model_name):
        try:
            if self.clf:
                self.logger.Information(f'IClassify::saving \'{model_name}\' model...')
                path = os.path.join(self.conf.working_path, model_name)
                ClassFile.save_model(path, self.clf)
        except Exception as e:
            self.logger.Error('IClassify::SAVE::{} - model -> {}'.format(e, model_name), sys.exc_info())
