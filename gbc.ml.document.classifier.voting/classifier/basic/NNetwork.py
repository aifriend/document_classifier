...
class NNetwork(IClassify):
    class CNNClassifier(ClassifierMixin, BaseMultilayerPerceptron):
        ...
        def save(self):
            try:
                ...
                self.logger.Debug('ERROR - NNetwork::SAVE::{}::{}'.format(e, self.conf.nn_model_path), sys.exc_info())
        ...
        def load(self):
            try:
                ...
                self.cnn_keras = None
                self.logger.Debug('ERROR - NNetwork::LOAD::{}::{}'.format(e, self.conf.nn_model_path), sys.exc_info())
    ...
    def _get_prediction(self, test):
        ...
        try:
            ...
            predicted = self.clf.cnn_keras.predict_generator(test, verbose=1, steps=(test.n / self.conf.nn_batch_size))
            ...
            self.logger.Debug('ERROR - NNetwork::get_prediction::{}'.format(e), sys.exc_info())
            return None
