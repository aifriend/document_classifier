...
from common.controller.AggregateFactory import AggregateFactory
                ...
                if model == AggregateFactory.ALL_VOTING:
                    ...
                    if mode == ClassifyFactory.TEXT:
                        try:
                            ...
                            if not text_classifier.check_source(data):
                            ...
                            elif not text_classifier.check_model(AggregateFactory.ALL_VOTING):
                                ...
                                ret = text_classifier.train(
                                    model=AggregateFactory.ALL_VOTING, data=data_decoded, nlp=nlp,
                    ...
                    if mode == ClassifyFactory.TEXT:
                        try:
                            if not text_classifier.check_source(data):
                                ...
                                nlp = SpacyModel.getInstance().getModel(lang)
                                ret = text_classifier.train(
                                    model=AggregateFactory.ALL_VOTING, data=data, nlp=nlp, dictionary=dictionary)
                                response = stats.from_json(ret)
