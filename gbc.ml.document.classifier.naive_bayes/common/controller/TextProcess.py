...
class TextProcess:
    ...
    def transform_data_s3(self, tfidfFile, s3Service, file_data=''):
        if self.encoder is None:
            self.load_vectorizer_model_s3(tfidfFile, s3Service)
        doc = self.process(NlpUtils.clean_text(file_data), 'none')
        vector = self.get_tfidf_from_vectorizer_s3(tfidfFile, doc.grams, s3Service)
        if vector is not None:
            return vector
        return None

    def transform_text(self, text, path="FILE_DATA"):
    ...
    def transform_text_s3(self, text, tfidf_file, s3_service):
        doc = self.process(NlpUtils.clean_text(text), 'none')
        # self.logger.Information(doc.grams)
