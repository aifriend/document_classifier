...
class TextClass:
    ...
    def train_by(self, model, data, nlp, dictionary):
        ...
        if nlp:
            ...
            if model == ClassifyFactory.NAIVE_BAYES_MULTI:
                ...
                response = self.classify.launch_naive_bayes_multinomial(X_train, y_train, X_test, True)
                self.classify.show_metrics(model, y_test, response, stats=stats)
            elif model == ClassifyFactory.NAIVE_BAYES_COMPLEMENT:
                self.logger.Information(f'TextClass - training {model}...')
                response = self.classify.launch_naive_bayes_complement(X_train, y_train, X_test, True)
                self.classify.show_metrics(model, y_test, response, stats=stats)

            else:
                response = []
                stats.info = 'Unsuitable training model. ' \
                             'Should be one of: NAIVE_BAYES_MULTI | NAIVE_BAYES_COMPLEMENT'
    ...
    def train_by_s3(self, model, nlp, dictionary, s3_service, s3_files):
        ...
        for f in v_list:
            ...
            gram_info = ast.literal_eval(s3Content)
        ...
        if nlp:
            ...
            # Basic classifiers
            if model == ClassifyFactory.NAIVE_BAYES_MULTI:
                response = self.classify.launch_naive_bayes_multinomial(X_train, y_train, X_test, True, s3_service)
                self.classify.show_metrics(model, y_test, response, stats=stats)
            elif model == ClassifyFactory.NAIVE_BAYES_COMPLEMENT:
                response = self.classify.launch_naive_bayes_complement(X_train, y_train, X_test, True, s3_service)
                self.classify.show_metrics(model, y_test, response, stats=stats)
    ...
    def predict_by(self, model, nlp, data=None, file_data=None, file_url=None):
        ...
        if nlp:
            ...
            if model == ClassifyFactory.NAIVE_BAYES_MULTI:
                ...
                response = self.classify.launch_naive_bayes_multinomial(None, None, X_test, False)
                self.classify.show_metrics(model, None, response, stats=stats)
            elif model == ClassifyFactory.NAIVE_BAYES_COMPLEMENT:
                self.logger.Information(f'TextClass - {model} prediction...')
                response = self.classify.launch_naive_bayes_complement(None, None, X_test, False)
                self.classify.show_metrics(model, None, response, stats=stats)
    ...
    def predict_by_s3(self, model, nlp, tfidf, data=None, s3Service=None):
        self._initialize(nlp=nlp)

        stats = Stats()
        response = []
        if nlp:
            vector_list = self.pre_process.transform_text_s3(data, tfidf, s3Service)
            ...
            # Basic classifiers
            if model == ClassifyFactory.NAIVE_BAYES_MULTI:
                response = self.classify.launch_naive_bayes_multinomial(None, None, X_test, False, s3Service=s3Service)
                self.classify.show_metrics(model, None, response, stats=stats)
            elif model == ClassifyFactory.NAIVE_BAYES_COMPLEMENT:
                response = self.classify.launch_naive_bayes_complement(None, None, X_test, False, s3Service=s3Service)
                self.classify.show_metrics(model, None, response, stats=stats)
    ...
    def check_model(model=None):
        ...
        if not (model == ClassifyFactory.NAIVE_BAYES_MULTI or model == ClassifyFactory.NAIVE_BAYES_COMPLEMENT):
            return False
