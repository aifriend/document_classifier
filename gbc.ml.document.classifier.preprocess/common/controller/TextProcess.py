...
class TextProcess:
    ...
    def process(self, text, kind='none', path='', s3Service=None):
        """
        process texts
        ...
        if not s3Service:
            ...
            sorted_list = sorted(list(xdoc.grams))
            gram_text = base64.b64encode(bytes(str(sorted_list), 'utf-8'))
            s3Service.upload_file(path + '.gram', gram_text)
        return xdoc
    ...
    def _do_pre_process(self, q, result):  # q:[[index, text, kind, path], ...]
        ...
        while not q.empty():
            ...
            try:
                ...
                data = self.process(work[1], work[2], work[3], work[4])
                result[work[0]] = data  # Store data back at correct index
                self.logger.Information("Done..." + str(work[0]) + " " + str(work[2]) + " " + str(work[3]) + " ")
        ...
        while i < total:
            ...
            for _ in range(self.conf.pre_process_batch_size):
                if h < total:
                    ...
                    q.put((counter, text, category, f, None))
    ...
    def pre_process_batches_s3(self, s3Service, all_docs):
        """
        Process all txt files in batches to get the .grams
        ...
        while i < total:
            ...
            for j in range(self.conf.pre_process_batch_size):
                if h < total:
                    ...
                    # Obtain txt
                    try:
                        s3Service.get_byte_file(f.Key + ".gram")
                        self.logger.Information(f'doc {f.Key} has the gram, skipping his process...')
                    except:
                        text = s3Service.get_txt_file(f.Key)
                        text = NlpUtils.clean_text(text)

                        self.logger.Information('doc %s to q' % (counter + 1))
                        q.put((counter, text, category, f.Key, s3Service))
    ...
    def create_full_dataset_vectorizer_s3(self, s3Service, s3Files):
        """
        ...
        for f in v_list:
            s3Content = s3Service.get_txt_file(f.Key)
        ...
        path = s3Service.domain + "/" + self.conf.vectorizer
        s3Service.upload_file(path, pickle_byte_obj)
        print("Success")
