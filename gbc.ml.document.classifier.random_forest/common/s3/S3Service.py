...
class S3Service(object):
    ...
    def get_files_from_s3(self, altDomain=None):
        ...
        try:
            ...
            resp = self.s3Session.list_objects_v2(Bucket=self.bucket, Prefix=domain)

            documents = []
            for doc in resp['Contents']:
                documents.append(S3File(doc))
            return documents
        except:
            raise Exception(f"No files found on bucket {self.bucket}, key {domain}")
