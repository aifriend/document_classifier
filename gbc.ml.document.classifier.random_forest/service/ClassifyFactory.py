...
from classifier.basic.RandomForest import RandomForest
...
class ClassifyFactory:
    ...
    def __init__(self, conf):
        self.conf = conf
        self._random_forest = RandomForest(conf)
    ...
    def launch_random_forest(self, X_train=None, y_train=None, X_test=None, train=False):
        self.logger.Information('---< Random Forest >---')
        result = ''
        if train and X_train is not None and y_train is not None and X_test is not None:
            result = self._random_forest.train(X_train, y_train, X_test)
        elif not train and X_test is not None:
            result = self._random_forest.predict(X_test)
